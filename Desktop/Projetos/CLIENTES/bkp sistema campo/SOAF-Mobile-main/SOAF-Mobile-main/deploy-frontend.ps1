...
if ($Production) {
    ...
    Write-Host "[INFO] Pressione ENTER para usar a URL oficial: https://soaf-mobile-backend.onrender.com" -ForegroundColor Yellow
    $InputUrl = Read-Host "URL do backend"
    
    if (-not [string]::IsNullOrWhiteSpace($InputUrl)) { $BackendUrl = $InputUrl } else { $BackendUrl = "https://soaf-mobile-backend.onrender.com" }
...
# Verificação de segurança: Backend Local em Deploy Público
if ($BackendUrl -match "localhost|127.0.0.1") {
    Write-Host "================================================================" -ForegroundColor Red
    Write-Host " [ALERTA CRÍTICO] URL do Backend aponta para LOCALHOST!" -ForegroundColor Red
    Write-Host " O site publicado no Firebase NÃO conseguirá acessar os dados." -ForegroundColor Red
    Write-Host " O erro 'não encontrando dados' ocorre geralmente por isso." -ForegroundColor Red
    Write-Host "================================================================" -ForegroundColor Red
...
Write-Host "  Frontend: https://soaf-mobile.web.app" -ForegroundColor White
Write-Host "  Backend:  $BackendUrl" -ForegroundColor White
Write-Host ""
Write-Host "Próximos passos:" -ForegroundColor Yellow
Write-Host "  1. Acesse https://soaf-mobile.web.app" -ForegroundColor Gray
